...
compile_kernel(){
	...
	echo -e "${CY}Running Vitis $EMU_TYPE make for Histogram ACCELERATE kernel... ${NC}"

	(set -x; v++ -c -t $EMU_TYPE \
	--include src \
	$extraCommands \
	--platform $PLATFORM \
	-s --kernel histogram_HM \
	--kernel_frequency $FREQ \
	-R2 \
	src/histogram.cpp -o src/workload-histogram-$EMU_TYPE.xo) &
	PIDS="$PIDS $!"
	...
	--kernel_frequency $FREQ \
	-R2 \
	src/workload-mem-$EMU_TYPE.xo src/workload-histogram-$EMU_TYPE.xo -o src/workload-$EMU_TYPE.xclbin
...
then
	...
	-R1 src/workload-mem-hw.xo src/workload-histogram-hw.xo -o src/workload-hw.xclbin
...
then
	...
	echo -e "${CY}Copying Vitis HLS reports for Histogram ACCELERATE kernel... ${NC}"
	cp _x/workload-histogram-hw/histogram_HM/histogram_HM/solution/syn/report/*.rpt FPGArpt/
