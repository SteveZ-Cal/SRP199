...
int main(int argc, char* argv[]) {
    ...
    for (uint32_t i = 0; i < NUM_INPUTSIZES; i++){
        ...
        for (uint32_t curr_loop = 0; curr_loop < NUM_LOOPS; curr_loop++) {
            ...
            OCL_CHECK(err, ptr_cipherText_map = (uint8_t*)q.enqueueMapBuffer(buffer_cipherText, CL_TRUE, CL_MAP_READ, 0, size_in_bytes, nullptr, nullptr, &err));
            OCL_CHECK(err, q.finish());
        	...
        	OCL_CHECK(err, err = q.enqueueReadBuffer(buffer_cipherText, CL_TRUE, 0, size_in_bytes, ptr_cipherText_map, nullptr, nullptr));
            OCL_CHECK(err, q.finish());
            ...
            if(VERBOSE && curr_loop == NUM_LOOPS - 1){
                ...
                printf("Iteraion: [%d]\n", curr_loop);
                while (iter < Psize) {
                    printf("%.2X%.2X%.2X%.2X ", ptr_cipherText_map[iter], ptr_cipherText_map[iter + 1],
                            ptr_cipherText_map[iter + 2], ptr_cipherText_map[iter + 3]);
                    printf("%.2X%.2X%.2X%.2X ", ptr_cipherText_map[iter + 4], ptr_cipherText_map[iter + 5],
                            ptr_cipherText_map[iter + 6], ptr_cipherText_map[iter + 7]);
                    iter += 8;
                }
                printf("\n");
            }
